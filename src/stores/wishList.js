...
const wishList = defineStore('wishList', () => {
    ...
    const totalItems = computed(() => Object.keys(items).length);
    ...
    async function getIcon(product){
        if(!items[product.id]){
    ...
    function getWishListFromLocalStorage() {
        ...
        if (storedWishlist) {
            Object.assign(items, storedWishlist);
        }
    }

    return { items, totalItems, toggle, getIcon }
})
